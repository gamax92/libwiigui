...
InitVideo ()
{
	...
	xfb[0] = (unsigned int *) SYS_AllocateFramebuffer (vmode);
	xfb[1] = (unsigned int *) SYS_AllocateFramebuffer (vmode);
	DCInvalidateRange(xfb[0], VIDEO_GetFrameBufferSize(vmode));
	DCInvalidateRange(xfb[1], VIDEO_GetFrameBufferSize(vmode));
	xfb[0] = (unsigned int *) MEM_K0_TO_K1 (xfb[0]);
	xfb[1] = (unsigned int *) MEM_K0_TO_K1 (xfb[1]);
